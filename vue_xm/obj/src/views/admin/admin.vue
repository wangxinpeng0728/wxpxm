...
<template>
<div style="background:#f5f5f5;margin-left:-160px">
...
<el-table border  :data="list.slice((page-1)*pnum,page*pnum)" style="width: 100%;" >
    ...
    <el-button  @click="jias()"><el-table-column prop="status" label="状态" width="120"></el-table-column></el-button>
    <el-table-column prop="level" label="等级" width="200"></el-table-column>
...
<script>
import {getuser,userdel,useredit,aeditStatus,getusers} from '../../../request/api.js'
export default {
   name:"App",
     data(){
     return{
  ...
  methods:{
    deleteData(index){
       if(this.list[index].level == "超级管理员"){
    alert('无法对超级管理员进行操作')
    return
    }
    else{
    ...
    sou(){ 
       getusers({page:this.p,search:this.search}).then(res=>{
          this.list = res.item;
